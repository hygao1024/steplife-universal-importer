<p align="center">   <strong>用最快的方式，将三方数据轨迹导入到「一生足迹」中</strong> </p> 
...
这是一款轨迹记录软件。多次被苹果 AppStore 推荐，数据完全存储在手机本地，无服务器，无广告，值得信赖。
...
`Step4.` 导出后的数据放置 `source_data/ovital`，在对应目录中打开终端，输入 `./main` 即可。
